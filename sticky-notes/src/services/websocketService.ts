...
class WebSocketService {
  ...
  connect(token: string, onConnect?: ConnectCallback, onError?: ErrorCallback): Promise<void> {
      ...
      this.client.onConnect = (frame) => {
        ...
        try {
          this.client!.publish({
            ...
            body: JSON.stringify({ sessionId: this.sessionId } as PresenceMessage)
          });
        } catch (error) {
  ...
  sendMessage(receiverId: string, content: string, messageType: string = 'DIRECT'): boolean {
    if (!this.connected) {
  ...
  subscribeToMessages(callback: MessageCallback, userId: string): any {
    if (!this.connected) {
      console.error('WebSocket not connected');
      return null;
    }
  ...
  subscribeToOnlineUsers(callback: OnlineUsersCallback): any {
  ...
  subscribeToBroadcast(callback: MessageCallback): any {
    if (!this.connected) {
      console.error('WebSocket not connected');
      return null;
    }
