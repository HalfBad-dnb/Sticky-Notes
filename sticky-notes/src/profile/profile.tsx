...
import { User, Note, EmbeddedApp } from "../types/index";

// Extended Note interface with additional properties used in profile
...
import "../App.css";
import "./Profile.css";
import NotesManagementModal from "./NotesManagementModal.jsx";
...
import Disclaimers from '../components/common/Disclaimers.jsx';
import BoardNavigation from '../components/common/BoardNavigation.jsx';
...
const Profile = () => {
  ...
  const { containerStyle, cardStyle } = useMemoizedStyles(theme, isMobile);
    ...
    try {
      ...
      if (response.data) {
        ...
        const profileNotes = Array.isArray(response.data) 
          ? response.data.filter((note: any) => note.boardType === 'profile')
          : [];
  ...
  const fetchProfileNotes = useCallback(() => {
    ...
    axios.get(url.toString())
      ...
      .then(notes => {
        ...
        const validNotes = notes.filter((note: Note) => note && note.id);
        
        // Filter by privacy setting if needed
        let processedNotes = validNotes;
        if (isPrivate) {
          processedNotes = validNotes.filter((note: Note) => (note as any).isPrivate === true);
        }
        
        // Assign z-index values based on current highest z-index
        const notesWithZIndex = processedNotes.map((note: ExtendedNote, index: number) => ({
          ...note,
          zIndex: (note.zIndex || 1000) + index
        }));
        
        // Update highest z-index if needed
        const maxZIndex = Math.max(...notesWithZIndex.map((note: ExtendedNote) => note.zIndex || 1000));
  ...
  const addEmbeddedApp = useCallback((type: 'youtube' | 'spotify' | 'soundcloud') => {
  ...
  const updateAppPosition = useCallback((appId: string | number, x: number, y: number) => {
    setEmbeddedApps(prev => 
      prev.map(app => 
  ...
  const updateAppUrl = useCallback((appId: string | number, url: string) => {
    setEmbeddedApps(prev =>
      prev.map(app =>
  ...
  const removeEmbeddedApp = useCallback((appId: string | number) => {
    setEmbeddedApps(prev => prev.filter(app => app.id !== appId));
  }, []);

  const toggleMinimizeApp = useCallback((appId: string | number) => {
    setEmbeddedApps(prev => 
      prev.map(app => 
  ...
  const DraggableEmbeddedApp = ({ app, onRemove, onToggleMinimize }: { 
    ...
    onToggleMinimize: (appId: string | number) => void; 
  }) => {
    ...
    const handleMouseDown = useCallback((e: React.MouseEvent) => {
      ...
      const onMouseMove = (e) => {
        ...
        const boundedX = Math.max(0, Math.min(window.innerWidth - (app.width || 400), x));
        const boundedY = Math.max(0, Math.min(window.innerHeight - (app.height || 300), y));
    ...
    const appStyle = {
      ...
      width: `${app.width || 400}px`,
      height: app.isMinimized ? '40px' : `${app.height || 300}px`,
    ...
    const iframeStyle = {
      width: '100%',
      height: app.isMinimized ? '0' : `${(app.height || 300) - 40}px`,
      border: 'none',
  ...
  const handleDelete = useCallback(async (id: string) => {
    setNoteToDelete(notes.find(note => note.id === id) || null);
                ...
                {filteredNotes.length === 0 ? (
                  ...
                  <div className="notes-container" style={getBoardStyle()}>
                    <div className="notes-items" style={{
                      ...
                      {filteredNotes.map((note, _index: number) => (
                        <StickyNote
                          key={note.id}
                          note={note}
                          onDrag={handleDrag}
