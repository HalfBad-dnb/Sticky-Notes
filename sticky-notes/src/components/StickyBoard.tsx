...
interface DraggableEmbeddedAppProps {
  app: EmbeddedApp;
  onRemove: (appId: number) => void;
  onToggleMinimize: (appId: number) => void;
}
...
const DraggableEmbeddedApp = ({ app, onRemove, onToggleMinimize }: DraggableEmbeddedAppProps) => {
  ...
  const applyPlaylist = (raw: string): void => {
    ...
    // TODO: pass url update up via callback when implemented
  };
...
const StickyBoard = ({ notes, setNotes, onDrag, onDone, onDelete, boardId }: StickyBoardProps) => {
  const [currentUser, setCurrentUser] = useState<{ username?: string } | null>(null);
  const [newNoteText, setNewNoteText] = useState('');
  const [error, setError] = useState<string | null>(null);
    const [activeNoteId, setActiveNoteId] = useState<string | number | null>(null);
  const [commentsOpenNoteId, setCommentsOpenNoteId] = useState<string | number | null>(null);
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
          ...
          if ((target as Element).classList.contains('menu-overlay') && (target as Element).classList.contains('active')) {
            handleMenuOpen();
          }
        }
      });
  ...
  const handleDoneWithBackend = useCallback((noteId: string | number) => {
    if (onDone) {
      onDone(noteId);
    }
  }, [onDone]);

  const handleDeleteWithBackend = useCallback((noteId: string | number) => {
    if (onDelete) {
      onDelete(noteId);
    }
  }, [onDelete]);
  ...
  const addEmbeddedApp = useCallback((type: 'youtube' | 'soundcloud' | 'spotify'): void => {
  ...
  const removeEmbeddedApp = useCallback((appId: number): void => {
    setEmbeddedApps(prev => prev.filter(app => app.id !== appId));
  }, []);

  const toggleMinimizeApp = useCallback((appId: number): void => {
    setEmbeddedApps(prev => 
      prev.map(app => 
  ...
  const getNoteZIndex = (noteId: string | number, index: number): number => {
  ...
  const handleNoteClick = (noteId: string | number): void => {
    setActiveNoteId(noteId);
  };

  // Handle comments open to lower all other notes
  const handleCommentsOpen = (noteId: string | number): void => {
  ...
  const handleUpdateNote = useCallback((updatedNote: Note): void => {
    const noteData = {
      id: updatedNote.id,
  ...
  useEffect(() => {
    ...
    const apiUrl = `notes/boards/${boardId}/notes`;
  ...
  const addNote = useCallback(() => {
    ...
    const apiUrl = boardId ? `notes/boards/${boardId}/notes` : 'notes';
    fetch(getApiUrl(apiUrl), {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
      ...
      .then((savedNote) => {
        ...
        setNotes((prevNotes: Note[]) => [...prevNotes, savedNote]);
        setNewNoteText('');
        setError(null);
      })
      .catch((error) => {
  ...
  const handleDragWithBackend = useCallback((id: string | number, x: number, y: number): void => {
    onDrag(id, x, y);
    ...
    fetch(getApiUrl(`notes/${id}`), {
      ...
      .then((updatedNote) => {
        ...
        if (updatedNote) {
          setNotes((prevNotes: Note[]) =>
            prevNotes.map((note: Note) => (note.id === id ? updatedNote : note))
          );
        }
      })
      .catch((error) => {
  ...
  return (
    <div className="sticky-board fullscreen" style={{
      ...
      <div className="input-container" style={{
        ...
        gap: '15px',
        position: 'relative',
        zIndex: 1, // Match profile board z-index behavior
